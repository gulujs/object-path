...
export class ObjectPath {
  ...
  escape(key) {
    ...
    const quote = (!key.includes(CHAR_SINGLE_QUOTE) || key.includes(CHAR_DOUBLE_QUOTE))
      ? CHAR_SINGLE_QUOTE
      : CHAR_DOUBLE_QUOTE;
    return `${quote}${key.replace(RE_ESCAPE_QUOTE_MAP[quote], `\\${quote}`)}${quote}`;
  }
  ...
  unescape(key) {
    ...
    return key.substring(1, tail).replace(RE_UNESCAPE_QUOTE_MAP[quote], quote);
  }
...
Object.getOwnPropertyNames(ObjectPath.prototype).forEach((name) => {
  if (name === 'constructor') {
    return;
  }
  ObjectPath[name] = (...args) => objectPath[name](...args);
});
